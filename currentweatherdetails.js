 ...
 * Custumized by MMPieps https://github.com/MMPieps
 * MIT Licensed.
 */
 
Module.register("currentweatherdetails",{
	...
	// Define required scripts.
	getStyles: function() {
		return ["weather-icons.css", "currentweatherdetails.css"];
	},
	...
	getParams: function() {
		 ...
		 */
		params += "&cnt=" + (((this.config.numberofentries < 3) || (this.config.numberofentries > 16)) ? 7 : this.config.numberofentries+3);
	...
	processWeather: function(data) {
		...
		this.forecast = [];
		for (var i = 0, count = this.config.numberofentries; i < count; i++) {

			
			if(moment(data.list[i+1].dt, "X").isAfter(moment()))
			{
				var forecast = data.list[i];
				var preci = "---";
				if(forecast.rain != undefined)
				{
					preci = this.roundValue(forecast.rain["3h"]/3)+" l/h"	;
				}
				else if (forecast.snow != undefined)
				{
					preci =  this.roundValue(forecast.snow["3h"]/3)+" l/h";
				}
				else
				{
					preci = "---";
				...
				this.forecast.push({
					day: moment(forecast.dt, "X").format("HH:00"),
					icon: this.config.iconTable[forecast.weather[0].icon],
					maxTemp: this.roundValue(forecast.main.temp)+" °C",
					minTemp: preci
	...
	scheduleUpdate: function(delay) {
		...
		console.log(this.name + ": scheduleUpdate()");
